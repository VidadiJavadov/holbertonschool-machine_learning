...
class Random_Forest:
    ...
    def fit(self, explanatory, target, n_trees=None, verbose=0):
        ...
        for i in range(n_trees):
            tree = Decision_Tree(max_depth=self.max_depth, min_pop=self.min_pop, seed=self.seed + i)
    ...
    def predict(self, explanatory):
        ...
        all_preds = np.array([predict_func(explanatory) for predict_func in self.numpy_predicts])
